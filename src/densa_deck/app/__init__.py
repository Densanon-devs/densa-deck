"""Desktop app wrapper for the Densa Deck.
...
Two entry points:
  - `from densa_deck.app.api import AppApi` — the Python API exposed
    to JS via pywebview's bridge. Testable in isolation (no pywebview).
  - `densa_deck.app.main:run()` — creates the webview window and
