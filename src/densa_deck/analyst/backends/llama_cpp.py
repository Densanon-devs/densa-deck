...
densa_deck.analyst. What we need from PIE's toolbox is the in-process
...
Model search path (first hit wins):
  ...
  2. `~/.densa-deck/models/analyst.gguf`

Recommended default: Llama 3.2 1B Instruct Q4_K_M (~770 MB). Configure via
`densa-deck analyst pull llama-3.2-1b` (future subcommand) or drop the file
at the default path.
"""
