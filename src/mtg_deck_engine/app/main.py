...
Run via `mtg-engine app`, which calls `run()`. pywebview is an optional
...
def run(debug: bool = False):
    ...
    # the installer shows "MTG Deck Engine" but the running window adds
    # version so users who installed via different channels know what's up.
    try:
        from mtg_deck_engine import __version__ as version
        title = f"MTG Deck Engine — v{version}"
    except ImportError:
        title = "MTG Deck Engine"
...
def _print_install_hint():
    ...
    print("    pip install 'mtg-deck-engine[desktop]'", file=sys.stderr)
    print("", file=sys.stderr)
