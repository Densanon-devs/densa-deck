...
from mtg_deck_engine.analysis.static import analyze_deck as run_static_analysis
from mtg_deck_engine.data.database import CardDatabase
from mtg_deck_engine.deck.parser import parse_decklist as parse_auto
...
from mtg_deck_engine.models import Format, Zone
from mtg_deck_engine.tiers import Tier, get_user_tier
...
class AppApi:
    ...
    default matches the CLI — `~/.mtg-deck-engine/cards.db` for the card
    database and `~/.mtg-deck-engine/versions.db` for deck snapshots.
    """
    ...
    def __init__(self, db_path: Path | str | None = None, version_db_path: Path | str | None = None,
        ...
        # the card database so everything lives under ~/.mtg-deck-engine/.
        self._state_path = (
            Path(state_path) if state_path
            else Path.home() / ".mtg-deck-engine" / "app_state.json"
        )
        ...
        self._session_path = (
            ...
            else Path.home() / ".mtg-deck-engine" / "coach_sessions.json"
        )
    ...
    def _load_coach_sessions(self):
        ...
        from mtg_deck_engine.analyst.coach import CoachSession, CoachTurn
        for item in payload:
    ...
    def import_deck_from_url(self, url: str) -> dict:
        ...
        into the textarea."""
        import asyncio
        from mtg_deck_engine.deck.url_import import detect_url, fetch_from_url
    ...
    def get_current_version(self) -> dict:
        ...
        from mtg_deck_engine import __version__ as v
        return {"version": v}

    @_safe
    def check_for_updates(self, url: str = "https://toolkit.densanon.com/mtg-engine-version.json") -> dict:
        ...
        from mtg_deck_engine import __version__ as current
        try:
    ...
    def coach_start(self, deck_id: str | None = None, decklist_text: str | None = None,
        ...
        """
        from mtg_deck_engine.analysis.power_level import estimate_power_level
        from mtg_deck_engine.analysis.static import analyze_deck as _analyze
        from mtg_deck_engine.analyst.coach import CoachSession, build_deck_sheet
        from mtg_deck_engine.formats.profiles import detect_archetype
    ...
    def coach_ask(self, token: str, question: str) -> dict:
        ...
        """
        from mtg_deck_engine.analyst.coach import coach_step
        with self._coach_lock:
    ...
    def _do_ingest(self, force: bool):
        ...
        try:
            ...
            from mtg_deck_engine.data.scryfall import (
    ...
    def _do_analyst_pull(self, model_key: str):
        ...
        try:
            ...
            from mtg_deck_engine.cli import _ANALYST_MODELS
    ...
    def run_gauntlet(
        ...
        from mtg_deck_engine.matchup.gauntlet import run_gauntlet as _run
        deck = self._build_deck(decklist_text, format_, name)
