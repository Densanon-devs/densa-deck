...
import type { CardQuery, CatalogueCard } from '../lib/protocol.ts';
...
export function CardBrowser({ state, onPick, onClose, countFor }: Props) {
  ...
  const run = useCallback(async () => {
    const query: CardQuery = { limit: 60 };
    ...
    if (!term && !colours.length && !types.length && !ownedOnly) {
      setProblem('Pick a colour or a type, or type part of a name.');
      return;
    }
  ...
  }, [name, colours, types, ownedOnly, state]);
  ...
  useEffect(() => {
    if (!colours.length && !types.length && !ownedOnly && !name.trim()) return;
    void run().catch(reporting('searching', setProblem));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [colours, types, ownedOnly]);
