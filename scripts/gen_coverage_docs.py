...
def build() -> str:
    ...
    w("### Rulings (opt-in)")
    w("")
    w("Wizards' official per-card rulings are a separate ~5 MB Scryfall bulk")
    w("file. They are **not** part of the card ingest and are never downloaded")
    w("automatically — nothing in the simulator or validator depends on them, so")
    w("they stay opt-in and are removable in one step.")
    w("")
    w("| Action | CLI | Desktop |")
    w("|--------|-----|---------|")
    w("| Check status | `densa-deck rulings status` | Settings → Official rulings |")
    w("| Opt in | `densa-deck rulings download` | **Download rulings** |")
    w("| Read for a card | `densa-deck rulings show \"Doubling Season\"` | — |")
    w("| Opt back out | `densa-deck rulings remove` | **Remove** |")
    w("")
    w("Stored in its own `~/.densa-deck/rulings.db`, so opting out is a file")
    w("delete and users who never opt in carry no extra weight. Rulings text is")
    w("© Wizards of the Coast, distributed via Scryfall; both are credited")
    w("wherever rulings are shown.")
    w("")
